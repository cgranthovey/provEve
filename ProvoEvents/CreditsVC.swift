...
class CreditsVC: GeneralVC, UITableViewDelegate, UITableViewDataSource, UITextViewDelegate {
    ...
    func makeFlatIconCredit(_ strTotal: String, imgString: String, url: String, urlWebsite: String = "http://www.flaticon.com/"){
        ...
        attString.addAttribute(NSAttributedStringKey.link, value: url, range: NSRange(location: 0, length: count))

        if urlWebsite != ""{
            let beginingOfWebsite = count + 6
            attString.addAttribute(NSAttributedStringKey.link, value: urlWebsite, range:  NSRange(location: beginingOfWebsite, length: strTotal.characters.count - beginingOfWebsite))
        }
