...
//        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AddEventVC.keyboardNotification(_:)), name: UIKeyboardWillChangeFrameNotification, object: nil)
            ...
            if let keyboardHeight = keyboardRect?.height{
                ...
                if activeTextField != nil{
                    ...
                    if !CGRectContainsPoint(aRect, point){
                        performSelector(#selector(AddEventVC.moveTextFieldIntoView), withObject: nil, afterDelay: 0.1)
                    }
    ...
    func moveTextFieldIntoView(){
        print("none")
        if activeTextField != nil{
            scrollView.setContentOffset(CGPointMake(0, activeTextField.frame.origin.y - myKeyBoardHeight - 50), animated: true)
        } else if activeTextView != nil{
            scrollView.setContentOffset(CGPointMake(0, activeTextView.frame.origin.y - myKeyBoardHeight - 50), animated: true)
        }
